...
import { Loader2, ArrowLeft, Download, Share2, Settings2, Film, MessageSquare, MinusCircle, UserCircle } from 'lucide-react';
...
export default function OwnerCreationDetailPage() {
  const params = useParams();
  const router = useRouter();
  const { user, userDoc } = useAuth(); // userDoc for role check
  ...
  useEffect(() => {
    const fetchCreation = async () => {
      if (!user || !creationId || userDoc?.role !== 'owner') {
        setIsLoading(false);
        if (!user) router.push('/owner-login');
        else if (userDoc?.role !== 'owner') router.push('/dashboard'); // or access denied
        return;
      }
      setIsLoading(true);
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Owner can view any creation if logic is changed, but for now, check if it's their own.
          // This check might be redundant if owner-dashboard/creations already filters.
          if (data.userId !== user.uid) { 
             toast({ variant: 'destructive', title: 'Access Denied', description: 'This creation does not belong to the logged-in owner account.' });
             router.push('/owner-dashboard/creations');
             return;
          }
          setCreation({
            id: docSnap.id,
    ...
    if (userDoc) { // Ensure userDoc is loaded before fetching
        fetchCreation();
    }
  }, [creationId, user, userDoc, router, toast]);
  ...
  const handleShare = (platform: 'twitter' | 'facebook') => {
    if (!creation) return;
    const text = encodeURIComponent(`Check out my AI creation from Luxe Muse (Owner): ${creation.prompt}`);
    const shareableUrl = encodeURIComponent(window.location.href); 
  ...
  if (isLoading || !userDoc) { // Also wait for userDoc
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)]">
        <Loader2 className="h-16 w-16 animate-spin text-primary mb-4" />
        <p className="text-muted-foreground">Loading owner creation details...</p>
      </div>
    );
  }
  ...
  if (!creation) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)]">
        ...
        <Button asChild>
          <Link href="/owner-dashboard/creations">
            <ArrowLeft className="mr-2 h-4 w-4" /> Back to Owner Creations
          </Link>
        </Button>
      </div>
    );
         ...
         <CardHeader className="p-0">
          <div className="relative w-full bg-muted" style={{ aspectRatio: `${creation.imageWidth || 1}/${creation.imageHeight || 1}`}}>
            <Image
              ...
              priority
              data-ai-hint="detailed creation owner"
            />
          </div>
        </CardHeader>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={() => handleShare('facebook')} aria-label="Share on Facebook">
             ...
             <Button variant="outline" disabled>
              <Share2 className="mr-2 h-4 w-4" /> More Sharing (Soon)
            </Button>
          </div>
        </CardFooter>
      </Card>
