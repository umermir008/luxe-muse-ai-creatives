...
import { Settings as SettingsIcon, ImageUp, UserCircle, Loader2, CheckCircle, ShieldCheck } from 'lucide-react';
import { useState, useEffect, ChangeEvent, FormEvent } from 'react';
import { useToast } from '@/hooks/use-toast';
import { storage, db, auth as firebaseAuth } from '@/firebase-config';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { doc, updateDoc } from 'firebase/firestore'; // Use updateDoc for merging
...
export default function OwnerSettingsPage() {
     ...
     return (
      <div className="text-center py-12">
        ...
        <p className="text-lg text-foreground/80 mb-8 max-w-md mx-auto">
          You must be logged in as an owner to manage these settings.
        </p>
  ...
  return (
    <div className="space-y-8">
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-3xl font-headline text-primary flex items-center">
            <SettingsIcon className="mr-3 h-8 w-8" /> Owner Account Settings
          </CardTitle>
          <CardDescription className="text-lg text-foreground/80">
            Manage your owner profile information and application preferences.
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xl flex items-center"><ShieldCheck className="mr-2 h-5 w-5 text-primary"/>Owner Profile</CardTitle>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input id="role" type="text" value={userDoc?.role || 'N/A'} disabled className="bg-input/50 capitalize" />
          </div>
            
        </CardContent>
        <CardFooter>
           <p className="text-sm text-muted-foreground">
              Owner-specific preferences and application-wide settings will appear here.
            </p>
        </CardFooter>
      </Card>
    </div>
